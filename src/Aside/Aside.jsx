...
const Aside = () => {
  ...
  return (
    <>
      <button
        className={`${menuMobile ? "block fixed top-8 left-4 text-3xl text-white z-500 cursor-pointer bg-black p-3 rounded-full" : "hidden"} ${openMenu && menuMobile ? "hidden" : ""} `}
      ...
      <div
        className={`${openMenu && menuMobile ? "fixed w-full z-50 bg-black/65 overflow-hidden " : "relative bg-transparent"} ${!openMenu && menuMobile ? "absolute" : ""} col-start-1 row-start-1 z-20 `}
      >
        <aside
